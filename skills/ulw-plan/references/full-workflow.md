...
The high-accuracy review is DUAL and both passes must return OKAY before handoff: (1) the native `momus` reviewer subagent, and (2) an independent Claude Code CLI review on gpt-5.5 at xhigh reasoning, run in a disposable isolated workspace and `LAZY_CLAUDECODE_HOME` with the harness's normal approval and sandbox policy. Do not add flags that disable approvals or sandboxing. Fix every cited issue and resubmit BOTH fresh until each approves. CLEAR: runs only if the user opts in at delivery. UNCLEAR: runs automatically unless Classify=Trivial.
