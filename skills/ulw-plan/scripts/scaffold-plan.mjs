...
// two things genuinely not guaranteed on native Windows across the lcc harnesses.
//
...
// own writes to resolve under .lazy-claudecode/ (it does not, and cannot, contain other Bash
// commands; it only guarantees the mandated generator never escapes .lazy-claudecode). Mirrors
// packages/lazy-claudecode/src/hooks/prometheus-md-only/path-policy.ts.
...
// Resolve a project-relative path and confine it under .lazy-claudecode/ - the script's own
...
export function resolveSafeOmoPath(cwd, relPath) {
	...
	if (!/(^|[/\\])\.lazy-claudecode([/\\]|$)/i.test(rel)) {
		throw new Error(`refused: ulw-plan may only write under .lazy-claudecode/: ${relPath}`);
	}
...
async function assertSafeWriteParent(cwd, target) {
	...
	const lccRoot = resolve(cwd, ".lazy-claudecode");
	const parent = dirname(target);
	assertContainedPath(workspaceRoot, parent, `refused: path escapes the workspace root: ${target}`);
	assertContainedPath(lccRoot, parent, `refused: ulw-plan may only write under .lazy-claudecode/: ${target}`);
	await mkdirWithoutSymlinks(parent, workspaceRoot);
	const lccReal = await realpath(lccRoot);
	const parentReal = await realpath(parent);
	assertContainedPath(workspaceReal, parentReal, `refused: path escapes the workspace root through symlinks: ${target}`);
	assertContainedPath(lccReal, parentReal, `refused: ulw-plan may only write under .lazy-claudecode/ through real paths: ${target}`);
}
...
status: drafting
intent: ${intent}
pending-action: write .lazy-claudecode/plans/${slug}.md
...
- Evidence: .lazy-claudecode/evidence/task-<N>-${slug}.<ext>
...
- [ ] 1. <title>
  ...
  QA scenarios (name the exact tool + invocation): happy + failure, Evidence .lazy-claudecode/evidence/task-1-${slug}.<ext>
...
export async function scaffold(cwd, { slug, intent, reset = false, force = false }) {
	const draftRel = join(".lazy-claudecode", "drafts", `${slug}.md`);
	const planRel = join(".lazy-claudecode", "plans", `${slug}.md`);
