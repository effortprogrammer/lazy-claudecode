...
# --- LCC runtime resolution (vendored patch) ---

def lcc_env_binary() -> Optional[Path]:
    raw_path = os.environ.get("LCC_AST_GREP_SG_PATH")
    if not raw_path:
        return None
...
def resolve_binary() -> Optional[Path]:
    ...
    1. LCC_AST_GREP_SG_PATH override
    2. LCC runtime dirs
    3. Cached binary in <skill>/bin/
    4. PATH (via shutil.which)
    5. Homebrew default paths
    """
    for fn in (lcc_env_binary, lcc_runtime_binary, cached_binary, which_binary, homebrew_binary):
        result = fn()
        if result:
            return result
    return None
