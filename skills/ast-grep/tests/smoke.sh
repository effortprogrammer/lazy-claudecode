...
# Given LCC_AST_GREP_SG_PATH points at a fake executable.
# When doctor resolves ast-grep, then it reports that exact path.
FAKE_ENV_SG="$OUTPUT_DIR/fake-env/sg"
fake_sg "$FAKE_ENV_SG"
LCC_AST_GREP_SG_PATH="$FAKE_ENV_SG" $HELPER doctor > "$OUTPUT_DIR/lcc-env.out" 2>&1
grep -Fq "ast-grep binary: $FAKE_ENV_SG" "$OUTPUT_DIR/lcc-env.out" || fail "LCC_AST_GREP_SG_PATH was not preferred: $(cat "$OUTPUT_DIR/lcc-env.out")"
pass "LCC_AST_GREP_SG_PATH resolves first"

# Given HOME has an LCC runtime sg executable.
# When doctor resolves ast-grep, then it reports the HOME runtime path.
RUNTIME_HOME="$OUTPUT_DIR/home"
RUNTIME_SLUG="$(lcc_runtime_slug)"
RUNTIME_BIN="sg"
case "$RUNTIME_SLUG" in
  win32-*) RUNTIME_BIN="sg.exe" ;;
esac
FAKE_RUNTIME_SG="$RUNTIME_HOME/.lazy-claudecode/runtime/ast-grep/$RUNTIME_SLUG/$RUNTIME_BIN"
fake_sg "$FAKE_RUNTIME_SG"
HOME="$RUNTIME_HOME" CODEX_HOME= LCC_AST_GREP_SG_PATH= $HELPER doctor > "$OUTPUT_DIR/lcc-runtime.out" 2>&1
grep -Fq "ast-grep binary: $FAKE_RUNTIME_SG" "$OUTPUT_DIR/lcc-runtime.out" || fail "LCC HOME runtime was not resolved: $(cat "$OUTPUT_DIR/lcc-runtime.out")"
pass "LCC HOME runtime resolves before standalone fallback"
