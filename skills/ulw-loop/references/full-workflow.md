...
## Goal
Deliver every goal in `.claude/ulw-loop/goals.json` end-to-end.
...
Audit each pass, fail, block, steering change, and checkpoint in `.claude/ulw-loop/ledger.jsonl`.
...
## Artifacts
- `.claude/ulw-loop/brief.md`: original brief and durable constraints.
- `.claude/ulw-loop/goals.json`: goals with embedded `successCriteria` per goal.
- `.claude/ulw-loop/ledger.jsonl`: append-only audit trail.
- Read artifacts before resuming, steering, or checkpointing.
- After any compaction or context loss, re-read brief + goals + ledger FIRST via `lazy-claudecode cat .claude/ulw-loop/ledger.jsonl` (or read the paths directly), then `lazy-claudecode ulw-loop status --json`, before any further action. Recover state from these artifacts; never re-plan from scratch or repeat completed work (re-hypothesizing inside a research goal is not a re-plan).
- Never invent state outside `.claude/ulw-loop` artifacts or `lazy-claudecode ulw-loop status --json`.
...
Resolve the CLI before the first command. If `lazy-claudecode` is absent from PATH or lacks `ulw-loop`, use the stable local installer bin or cached Claude Code component CLI — same CLI, so PATH absence is not a blocker. If PATH is empty, the fallback uses shell builtins and absolute Node locations before reporting guidance, recording the failure in `.claude/ulw-loop/bootstrap-notepad.md`.
```sh
CODEX_HOME="${CODEX_HOME:-$HOME/.claude-code}"
...
ULW_LOOP_CLI=
if command -v lazy-claudecode >/dev/null 2>&1 && lazy-claudecode ulw-loop help >/dev/null 2>&1; then
  ULW_LOOP_CLI=lazy-claudecode
elif [ -n "$ULW_LOOP_NODE" ]; then
  for candidate in "$HOME/.local/bin/lazy-claudecode" "$CODEX_HOME/bin/lazy-claudecode" "$CODEX_HOME"/plugins/cache/sisyphuslabs/lazy-claudecode/*/components/ulw-loop/dist/cli.js; do
  ...
  if [ -n "$ULW_LOOP_CLI" ] && [ -n "$ULW_LOOP_NODE" ]; then
    lazy-claudecode-cli() { "$ULW_LOOP_NODE" "$ULW_LOOP_CLI" "$@"; }
  fi
fi

if [ -z "${ULW_LOOP_CLI:-}" ]; then
  /bin/mkdir -p .claude/ulw-loop 2>/dev/null || mkdir -p .claude/ulw-loop 2>/dev/null || true
  NOTE="${NOTE:-.claude/ulw-loop/bootstrap-notepad.md}"
  printf '%s\n' "No ulw-loop-capable lazy-claudecode executable found; PATH lazy-claudecode may be the Claude Code CLI without the Claude Code ulw-loop subcommand, and cached ulw-loop CLI was not found under ${CODEX_HOME:-$HOME/.claude-code}." >> "$NOTE" 2>/dev/null || true
  printf '%s\n' "Install with npx lazyclaude-code-ai install or set CODEX_LOCAL_BIN_DIR to a PATH directory." >&2
fi
```
...
Run one form:
```sh
lazy-claudecode ulw-loop create-goals --brief "<brief>" --json
lazy-claudecode ulw-loop create-goals --brief-file <path> --json
cat <brief> | lazy-claudecode ulw-loop create-goals --from-stdin --json
```
...
`lazy-claudecode ulw-loop create-goals --session-id <new-id> ...`; use `--force`
...
### 3. Inspect state
Run `lazy-claudecode ulw-loop status --json`.
Read pending goals, criteria IDs, current ledger head, blockers, and aggregate Claude Code objective.

## Execution Loop
Loop per goal. Cap at 5 cycles per goal. Cap identical same-criterion failures at 3.

### Acquire Next Goal
1. Run `lazy-claudecode ulw-loop complete-goals --json` and read the handoff, including criteria.
2. Call `get_goal` and inspect active Claude Code state.
...
4. If retrying failed work, run `lazy-claudecode ulw-loop complete-goals --retry-failed --json`.
5. Never create a second Claude Code goal for the same aggregate objective.
...
8. RECORD exactly one result:
   - PASS: `lazy-claudecode ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status pass --evidence "<observable> | <cleanup receipt>" --json`
   - FAIL: `lazy-claudecode ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status fail --evidence "<observable> | <cleanup receipt>" --notes "<diagnosis>" --json`
   - BLOCKED: `lazy-claudecode ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status blocked --evidence "<observable>" --notes "<safety/blocker/leftover-state>" --json`
...
3. Run `lazy-claudecode ulw-loop checkpoint --goal-id <id> --status complete --evidence "<criteria evidence summary>" --claude-code-goal-json <snapshot> --json`.
...
5. If review remains blocked, run `lazy-claudecode ulw-loop record-review-blockers --goal-id <id> --title "<...>" --objective "<...>" --evidence "<review findings>" --claude-code-goal-json <snapshot> --json`.
6. If clean, checkpoint final completion:
```sh
lazy-claudecode ulw-loop checkpoint --goal-id <id> --status complete --evidence "<e2e evidence + manual QA notes>" --claude-code-goal-json <snapshot> --quality-gate-json <json-or-path> --json
```
`--quality-gate-json` shape:
```json
{
  "codeReview":{"by":"lazy-claudecode-reviewer","recommendation":"APPROVE","codeQualityStatus":"CLEAR","reportPath":"packages/lazy-claudecode/plugin/components/ulw-loop/test/fixtures/artifacts/code-review.md","evidence":"Diff review passed.","blockers":[]},
  "manualQa":{"by":"lazy-claudecode-qa-executor","status":"passed","evidence":"CLI and data surfaces passed.","surfaceEvidence":[{"id":"surface-cli-pass","criterionRef":"C1","surface":"cli","invocation":"lazy-claudecode ulw-loop checkpoint --quality-gate-json sample-quality-gate.json --json","verdict":"passed","artifactRefs":["artifact-cli-pass"]},{"id":"surface-data-pass","criterionRef":"C2","surface":"data","invocation":"diff -u before-ledger.json after-ledger.json","verdict":"passed","artifactRefs":["artifact-data-diff"]}],"adversarialCases":[{"id":"adv-malformed-input","criterionRef":"C3","scenario":"malformed gate input omits manual QA evidence","expectedBehavior":"validator rejects ULW_LOOP_QUALITY_GATE_INVALID","verdict":"passed","artifactRefs":["artifact-cli-reject"]}],"artifactRefs":[{"id":"artifact-cli-pass","kind":"cli-transcript","description":"CLI pass artifact.","path":"packages/lazy-claudecode/plugin/components/ulw-loop/test/fixtures/artifacts/cli-pass.txt"},{"id":"artifact-cli-reject","kind":"log","description":"Reject log artifact.","path":"packages/lazy-claudecode/plugin/components/ulw-loop/test/fixtures/artifacts/rejection.txt"},{"id":"artifact-data-diff","kind":"data-diff","description":"Data diff artifact.","path":"packages/lazy-claudecode/plugin/components/ulw-loop/test/fixtures/artifacts/data-diff.txt"}]},
  "gateReview":{"by":"lazy-claudecode-gate-reviewer","recommendation":"APPROVE","reportPath":"packages/lazy-claudecode/plugin/components/ulw-loop/test/fixtures/artifacts/gate-review.md","evidence":"Gate review passed.","blockers":[]},
  "iteration":{"fullRerun":true,"status":"passed","rerunCommands":["bunx vitest run packages/lazy-claudecode/plugin/components/ulw-loop/test/quality-gate-doc.test.ts"],"evidence":"Focused rerun passed."},
...
Command form: `lazy-claudecode ulw-loop steer --kind <kind> [<kind-specific-fields>] --evidence "<...>" --rationale "<...>" --json`.
Structured prompt directives accepted: `LAZY_CLAUDECODE_ULW_STEER: { ... }`, `lazy-claudecode.ulw-loop.steer: {...}`, `lazy-claudecode ulw-loop steer: {...}`.
...
6. Treat `.claude/ulw-loop/ledger.jsonl` as the durable audit trail; checkpoint after every success or failure.
7. Per-story Claude Code goal mode is opt-in only with `--claude-code-goal-mode per-story`; default is aggregate.
...
11. After completing an aggregate ulw-loop run, clear the Claude Code goal manually with `/goal clear` before starting another in the same session.
12. The shell command emits a model-facing handoff; only the Claude Code agent calls `get_goal`, `create_goal`, or `update_goal` tools.
...
- Claude Code `get_goal` reports a different active goal: checkpoint blocker, stop, surface.
