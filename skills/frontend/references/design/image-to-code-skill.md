---
name: image-to-code
description: Elite website image-to-code skill for Claude Code. For visually important web tasks, it must first generate the design image(s) itself, deeply analyze them, then implement the website to match them as closely as possible. In Claude Code, it must prefer large, readable, section-specific images instead of tiny compressed boards, generate fresh standalone images for sections or detail views instead of cropping old ones, avoid lazy under-generation, avoid cards-inside-cards-inside-cards UI, and keep the hero clean, spacious, readable, and visible on a small laptop.
---
...
- In Claude Code, bias strongly toward larger, more analyzable section images.
...
In Claude Code, prefer separate large images per section.

Default rule inside Claude Code:
...
Outside Claude Code, this skill may still allow more compact multi-section composition when appropriate.
Inside Claude Code, prioritize section clarity and extraction accuracy.
...
Inside Claude Code, treat each section as its own analyzable unit.
...
In Claude Code, these should usually become section-by-section images, not one compressed sheet.
...
17. Has Claude Code avoided compressing too many sections into one tiny image?
...
4. inside Claude Code, prefer one large image per section
...
Do not compress many sections into one unreadable image in Claude Code.
...
Interpretation:
- generate 8 separate section images in Claude Code
- one per section
...
Interpretation:
- generate 4 separate section images in Claude Code
...
Inside Claude Code, if the user wants multiple sections, prefer separate large section images instead of one compressed multi-section board, so text, spacing, typography, buttons, and colors can be extracted properly.
