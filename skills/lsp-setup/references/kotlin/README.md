...
Builtin — usually NO config needed (auto-resolved by extension). Configure only to set priority, init options, override extensions, or disable. Same JSON shape in `.claude-code/lsp-client.json` (Claude Code) AND `.claude-code/lsp.json` (Claude Code/lazy-claudecode):

```json
{ "lsp": { "kotlin-ls": { "priority": 100 } } }
```

For builtin ids in a PROJECT config, `command` is supplied automatically — only set `priority`/`initialization`/`extensions`/`disabled`/`env`. A fully custom (non-builtin) server with its own `command` must go in the USER config (`~/.claude-code/lsp-client.json`).
