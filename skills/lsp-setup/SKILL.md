...
- Claude Code harness → `.claude-code/lsp-client.json` (user: `~/.claude-code/lsp-client.json`)
- Claude Code/lazy-claudecode harness → `.claude-code/lsp.json` (also `.claude/lsp.json`)
...
- In a **project** config (`.claude-code/lsp-client.json`, `.claude-code/lsp.json`) an
...
- To define a **fully custom** (non-builtin) server with its own `command`, put
  it in the **user** config (`~/.claude-code/lsp-client.json`, or the path set by
...
`SKIP` = the engine source could not be located; run from inside the lazy-claudecode
