...
Fire ALL of these simultaneously using \`call_lcc_agent\`:
...
3. If \`~/.lazy-claudecode/teams/refactor-squad/config.json\` is missing, write it using the spec below.

**Team spec** (\`~/.lazy-claudecode/teams/refactor-squad/config.json\`):
...
The \`~/.lazy-claudecode/teams/refactor-squad/config.json\` declaration stays on disk; next session reuses it.
