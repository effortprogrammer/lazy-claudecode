---
name: teammode
description: "Claude Code-only team orchestration: run a named team of cooperating Claude Code threads with durable, script-managed state. MUST USE when the user asks Claude Code to create, run, coordinate, inspect, archive, or delete a team of threads/sessions, or to work on something as a team in parallel. The main session is always the leader; members are defined by a concrete part, ownership area, or perspective - never a vague job role; a bundled cross-platform script writes the .claude/teams state plus an auto-generated member field manual. Use a team when the work is not perfectly isolated but parallelizing helps, or when a task still needs exploration under a clear goal; use plain subagents when scope is perfectly isolated or the goal is ambiguous. Triggers: team mode, teammode, make a team, run as a team, team of agents, coordinate threads, parallel Claude Code threads, archive the team, delete the team."
---

# Teammode

Run a named team of cooperating Claude Code threads under one leader, with durable state on disk.
This is a Claude Code-only workflow. It is inspired by the lifecycle concerns in the
Yeachan-Heo/oh-my-claude-code team skill, but it does not copy that runtime model and never depends
on an external terminal runner - it coordinates through Claude Code's own thread tools plus a bundled
state script.
...
`init` creates `.claude/teams/{session_id}/` containing `team.json` (the single durable state file:
...
space). `{session_id}` is the leader's Claude Code session id when you can pass it via `--session`;
...
2. Create a durable thread per member with `claude-code_app.create_thread` - ALWAYS this tool for every
   ...
   exact title to use. If `claude-code_app.create_thread` accepts a working directory / cwd argument,
   set it to that member's worktree; otherwise the member's manual tells it to `cd` there first.
   Use `claude-code_app.set_thread_title` if the title did not land at creation.
...
Every team member is a real Claude Code thread created with `claude-code_app.create_thread` - this is strict,
...
re-opened with the `claude-code_app.*` thread tools - which defeats the entire point of a durable team.
A member only counts once you have `bind-thread`-ed it to a real `claude-code_app.create_thread` thread
id. If the thread-creation tool is unavailable, STOP and say so (see Stop rules); do not quietly
fall back to a spawned agent.

## Communication

Members push to you and to one another with `claude-code_app.send_message_to_thread`; you inspect their
state with `claude-code_app.read_thread`. So members can actually reach you, run `init` with
...
When a decision-complete plan already exists at `.claude/plans/<slug>.md` (from ulw-plan), execute its
...
- `archive` closes the team: notify each active member, copy anything useful into `artifacts/`,
  archive each member thread with `claude-code_app.set_thread_archived`, then `archive` flips the team
...
- `delete` removes `.claude/teams/{session_id}` and refuses while the team is unarchived or any member
...
- Stop if the requested operation needs Claude Code thread tools (create/read/send/title/archive) and
