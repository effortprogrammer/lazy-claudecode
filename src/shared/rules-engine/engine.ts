...
import { loadDynamicCandidates } from "./engine-dynamic-loader.ts";
import { loadStaticCandidates } from "./engine-static-loader.ts";
import type { DynamicMatchCache, Engine, EngineDeps } from "./engine-types.ts";
import { formatDynamicBlock, formatStaticBlock } from "./formatter.ts";
import { disabledSourcesFromConfig } from "./sources.ts";
import type { LoadedRule, PiRulesConfig, RuleDiagnostic, SessionState } from "./types.ts";

export type { Engine, EngineDeps } from "./engine-types.ts";
...
export function defaultConfig(): PiRulesConfig {
	return {
		...
		dynamicMaxRuleChars: DEFAULT_MAX_RULE_CHARS,
		dynamicMaxResultChars: DEFAULT_MAX_RESULT_CHARS,
		promptMaxRuleChars: DEFAULT_MAX_RULE_CHARS,
		promptMaxResultChars: DEFAULT_MAX_RESULT_CHARS,
		enabledSources: "auto",
	};
}
