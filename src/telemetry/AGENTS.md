...
- No Bun APIs. Runtime is Node only because Claude Code launches plugin hooks with Node.
...
- Hook output MUST stay empty (no `additionalContext`, no `systemMessage`). This component is observability-only and MUST NOT inject context into the Claude Code conversation.
- Constants in `src/product-identity.ts` MUST stay byte-equivalent with `packages/lazy-claudecode-claude-code/src/telemetry/product-identity.ts`. The cross-package equivalence test will fail otherwise.
- Do not couple this component back to lazy-claudecode internal source paths beyond what `cross-package-equivalence.test.ts` already asserts at the constants layer.
