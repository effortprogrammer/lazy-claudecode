...
import { parseGoalArg, readJsonInput, readValue } from "./cli-arg-parser.ts";
import { printJson, printStatus } from "./cli-output.ts";
import type {
	SteerUlwLoopResult,
...
function isKind(value: string | undefined): value is UlwLoopSteeringMutationKind {
...
export function parseSteeringKind(argv: readonly string[]): UlwLoopSteeringMutationKind {
	...
	return value === undefined
		? fail("Missing --kind.", "ULW_LOOP_STEERING_KIND_REQUIRED", { flag: "--kind" })
		: fail(`Invalid --kind: ${value}.`, "ULW_LOOP_STEERING_KIND_INVALID", {
				value,
...
async function children(
	...
	for (const item of raw) {
		...
		if (next === null)
			return fail(`${flag} entries require title/objective.`, "ULW_LOOP_STEERING_CHILD_INVALID", {
				flag,
			});
		parsed.push(next);
	}
...
async function stringArray(argv: readonly string[], flag: string): Promise<string[]> {
	...
	for (const item of raw) {
		if (typeof item !== "string")
			return fail(`${flag} entries must be strings.`, "ULW_LOOP_STEERING_STRING_ARRAY_REQUIRED", {
				flag,
			});
...
function model(value: string | undefined): UlwLoopSuccessCriterionUserModel | undefined {
	const trimmed = text(value, "--user-model");
	if (trimmed === undefined) return undefined;
	return isModel(trimmed)
		? trimmed
		: fail(`Invalid --user-model: ${trimmed}.`, "ULW_LOOP_STEERING_USER_MODEL_INVALID", {
				value: trimmed,
				expected: ULW_LOOP_SUCCESS_CRITERION_USER_MODELS,
			});
}
function neverKind(kind: never): never {
	return fail(`Unsupported steering kind: ${String(kind)}.`, "ULW_LOOP_STEERING_KIND_UNSUPPORTED", {
		kind,
	});
}
...
export function normalizeSteeringProposal(proposal: CliSteeringProposal): CliSteeringProposal {
	...
	return {
		...
		...(scenario === undefined ? {} : { scenario }),
		...(expectedEvidence === undefined ? {} : { expectedEvidence }),
		...(proposal.userModel === undefined ? {} : { userModel: proposal.userModel }),
	};
}
