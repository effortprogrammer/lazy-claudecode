...
import { appendLedger, readSteeringLedgerEntries, readUlwLoopPlan, withUlwLoopMutationLock, writePlan } from "./plan-io.ts";
import type {
	SteerUlwLoopResult,
	UlwLoopItem,
	UlwLoopLedgerEntry,
...
import { iso, ULW_LOOP_STEERING_MUTATION_KINDS, ULW_LOOP_SUCCESS_CRITERION_USER_MODELS } from "./types.ts";
...
export function parseUlwLoopSteeringDirective(text: string): UlwLoopSteeringProposal | null {
	const match = /(?:^|\s)(?:LAZY_CLAUDECODE_ULW_STEER|lazy-claudecode\.ulw-loop\.steer|lazy-claudecode ulw-loop steer):\s*([\s\S]+)$/u.exec(text);
