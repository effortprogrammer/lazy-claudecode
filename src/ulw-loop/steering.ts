...
import { isUlwLoopDone } from "./goal-status.ts";
import type { UlwLoopScope } from "./paths.ts";
import { seedDefaultSuccessCriteria } from "./plan-crud.ts";
import {
	appendLedger,
...
const isPlain = (value: unknown): value is object => isObject(value) && !Array.isArray(value);
...
const revised = (proposal: object, direct: string, nested: string): string | undefined =>
...
const children = (proposal: object): UlwLoopSteeringChildGoal[] =>
...
function weakens(value: unknown): boolean {
	...
	return (
		/\b(skip|bypass|weaken|remove|omit|auto[-\s]?complete|mark complete|complete faster)\b/.test(
			valueText,
...
function validateKind(
	...
	if (
		kind === "add_subgoal" &&
		(text(proposal, "title") === undefined || text(proposal, "objective") === undefined)
	)
...
export function parseUlwLoopSteeringDirective(text: string): UlwLoopSteeringProposal | null {
	const match =
		/(?:^|\s)(?:LAZY_CLAUDECODE_ULW_STEER|lazy-claudecode\.ulw-loop\.steer|lazy-claudecode ulw-loop steer):\s*([\s\S]+)$/u.exec(
			text,
		);
...
export async function steerUlwLoop(
	...
	return withUlwLoopMutationLock(repoRoot, scope, async () => {
		...
		await appendLedger(
			repoRoot,
			ledgerEntry(proposal, finalAudit, proposal.now?.toISOString() ?? iso()),
			scope,
		);
		return {
			plan: next,
