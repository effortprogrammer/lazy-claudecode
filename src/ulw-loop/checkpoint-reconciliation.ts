...
import { claudeCodeGoalMode, isFinalRunCompletionCandidate } from "./goal-status.ts";
import { type UlwLoopScope, ulwLoopBriefPath } from "./paths.ts";
import type { UlwLoopItem, UlwLoopPlan } from "./types.ts";
import { ULW_LOOP_DIR, ULW_LOOP_GOALS, ULW_LOOP_LEDGER } from "./types.ts";
...
function buildCompletedLegacyGoalRemediation(goal: UlwLoopItem): string {
	return [
		...
		`Record a non-terminal blocker with: lazy-claudecode ulw-loop checkpoint --goal-id ${goal.id} --status blocked --evidence "<completed legacy Claude Code goal blocks create_goal in this thread>" --claude-code-goal-json "<different completed get_goal JSON or path>".`,
		"Then continue only from a Claude Code goal context with no active/completed conflicting goal, in the same repo/worktree, and create the intended goal there.",
	].join(" ");
}
