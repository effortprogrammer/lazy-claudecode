...
function buildText(
	...
	return joinLines([
		...
		"",
		"Claude Code goal integration constraints:",
...
function modeConstraintLines(mode: UlwLoopClaude CodeGoalMode, isFinal: boolean): readonly string[] {
	if (mode === "per_story") {
		return [
			...
			"- If a different active Claude Code goal exists, finish/checkpoint that goal before starting this ulw-loop.",
			"- Work only this goal until its completion audit passes.",
		];
	}
	return [
		"- Claude Code goal = the whole lazy-claudecode ulw-loop run; OMO G001/G002/etc. = ledger stories.",
		"- First call get_goal. If no active goal exists, call create_goal with the aggregate payload below.",
		"- If get_goal reports the same aggregate objective as active, continue this OMO story without creating a new Claude Code goal.",
		"- If a different active or incomplete Claude Code goal exists, finish/checkpoint that goal before starting this ulw-loop.",
		isFinal
...
function finalSection(plan: UlwLoopPlan, goal: UlwLoopItem, isFinal: boolean, aggregate: boolean): string {
	...
	const blockerCommand = `lazy-claudecode ulw-loop record-review-blockers${option} --goal-id ${goal.id} --title "Resolve final code-review blockers" --objective "<blocker-resolution objective>" --evidence "<review findings>" --claude-code-goal-json "<active get_goal JSON or path>"`;
	const checkpointCommand = `lazy-claudecode ulw-loop checkpoint${option} --goal-id ${goal.id} --status complete --evidence "<targeted verification/manualQa/gateReview evidence>" --claude-code-goal-json "<fresh complete get_goal JSON or path>" --quality-gate-json "<quality gate JSON or path>"`;
	return joinLines([
