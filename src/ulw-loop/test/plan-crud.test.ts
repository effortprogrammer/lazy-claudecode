...
import { ulwLoopBriefPath, ulwLoopGoalsPath, ulwLoopLedgerPath } from "../src/paths.ts";
import {
	addUlwLoopGoal,
	createUlwLoopPlan,
...
describe("createUlwLoopPlan", () => {
	it("creates .claude/ulw-loop/{brief.md, goals.json, ledger.jsonl} in repoRoot", async () => {
...
describe("startNextUlwLoop", () => {
	...
	it("with retryFailed picks first failed (non-blocked) goal", async () => {
		...
		await mkdir(join(repoRoot, ".claude", "ulw-loop"), { recursive: true });
