...
describe("ulwLoopDir(repo)", () => {
	it("returns repo + '/.claude/ulw-loop'", () => {
		// when/then
		expect(ulwLoopDir("/repo")).toBe("/repo/.claude/ulw-loop");
	});

	it("#given a session id #when resolving the loop dir #then scopes artifacts under that session", () => {
		// when/then
		expect(ulwLoopDir("/repo", { sessionId: "sess_abc" })).toBe("/repo/.claude/ulw-loop/sess_abc");
	});
});
...
describe("ulw-loop*Path helpers", () => {
	it("compose artifact filenames under ulwLoopDir", () => {
		// when/then
		expect(ulwLoopBriefPath("/r")).toBe("/r/.claude/ulw-loop/brief.md");
		expect(ulwLoopGoalsPath("/r")).toBe("/r/.claude/ulw-loop/goals.json");
		expect(ulwLoopLedgerPath("/r")).toBe("/r/.claude/ulw-loop/ledger.jsonl");
	});
