		...
		goals: [
			{
				id: "G001",
				title: "Build hook",
				objective: "Apply safe steering directives from Claude Code hooks.",
				status: "pending",
				successCriteria: [],
				attempt: 0,
				createdAt: NOW,
...
describe("applyUserPromptUlwLoopSteering - LAZY_CLAUDECODE directive patterns", () => {
	...
	it("#given a Claude Code session id #when steering from a hook #then writes the session-scoped ledger", async () => {
		const repoRoot = await bootstrapPlanRepo();
	...
	it("processes lazy-claudecode.ulw-loop.steer: pattern", async () => {
		...
		const out = await applyUserPromptUlwLoopSteering(
			payload(
				'lazy-claudecode.ulw-loop.steer: {"kind":"annotate_ledger","source":"user_prompt_submit","evidence":"x","rationale":"y"}',
				repoRoot,
			),
		);
		expect(out).toContain("accepted");
...
describe("runPreToolUseGoalBudgetGuardCli", () => {
	it("#given Claude Code PreToolUse stdin with budgeted create_goal #when CLI hook runs #then it writes blocking JSON", async () => {
		// given
		const stdin = Readable.from([
