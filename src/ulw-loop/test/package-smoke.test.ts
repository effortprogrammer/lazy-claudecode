...
describe("package.json", () => {
	...
	it("ships the expected files for npm publish", async () => {
		...
		expect(files).not.toContain(".claude-code-plugin");
	});
});
...
describe("skills/ulw-loop/SKILL.md", () => {
	...
	it("#given Claude Code skill hinting #when ulw-loop skill metadata is inspected #then ulw-loop is the primary mention name", async () => {
	...
	it("#given Claude Code dollar hinting #when querying ulw-loop #then ulw-loop remains discoverable as an alias", async () => {
		const text = await readText("skills/ulw-loop/agents/openai.yaml");
	...
	it("#given PATH lazy-claudecode lacks ulw-loop #when bootstrap runs #then falls back to cached ulw-loop CLI", async () => {
		const text = await readText("skills/ulw-loop/references/full-workflow.md");
		const bootstrap = bootstrapScriptFrom(text);
		const root = await mkdtemp(join(tmpdir(), "lazy-claudecode-ulw-loop-bootstrap-"));
		try {
