...
const VALUE_FLAGS = new Set("--brief --brief-file --session-id --claude-code-goal-mode --goal --goal-id --criterion-id --status --evidence --notes --claude-code-goal-json --quality-gate-json --kind --rationale --title --objective --target-goal-id --source --after-json --directive-json --directive-file --idempotency-key".split(" "));
...
export async function parseClaudeCodeGoalJson(value: string | undefined): Promise<string | undefined> {
	...
	try { JSON.parse(raw); return raw; }
		...
		const message = error instanceof Error ? error.message : "unknown error";
		throw new UlwLoopError(`Invalid --claude-code-goal-json: ${message}`, "ULW_LOOP_CLAUDE_CODE_GOAL_JSON_INVALID", { cause: error });
	}
}
