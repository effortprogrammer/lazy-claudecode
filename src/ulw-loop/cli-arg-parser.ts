...
const VALUE_FLAGS = new Set(
	"--brief --brief-file --session-id --claude-code-goal-mode --goal --goal-id --criterion-id --status --evidence --notes --claude-code-goal-json --quality-gate-json --kind --rationale --title --objective --target-goal-id --source --after-json --directive-json --directive-file --idempotency-key".split(
		" ",
	),
);
const SUBCOMMANDS = new Set(
	"create-goals status complete-goals criteria record-evidence checkpoint steer add-goal record-review-blockers".split(
		" ",
	),
);
...
export async function readJsonInput(value: string | undefined): Promise<unknown | undefined> {
	...
	try {
		...
		throw new UlwLoopError(`Invalid JSON input: ${message}`, "ULW_LOOP_JSON_INPUT_INVALID", {
			cause: error,
		});
	}
}

export async function parseClaudeCodeGoalJson(
	...
	try {
		...
		throw new UlwLoopError(
			`Invalid --claude-code-goal-json: ${message}`,
			"ULW_LOOP_CLAUDE_CODE_GOAL_JSON_INVALID",
			{ cause: error },
		);
	}
}
...
export function parseRecordEvidenceArgs(argv: readonly string[]): RecordEvidenceCliArgs {
	const result = {
		...
		criterionId: required(argv, "--criterion-id", "ULW_LOOP_CRITERION_ID_REQUIRED"),
		status: evidenceStatus(required(argv, "--status", "ULW_LOOP_EVIDENCE_STATUS_REQUIRED")),
