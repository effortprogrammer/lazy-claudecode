import type { UlwLoopClaudeCodeGoalMode, UlwLoopItem, UlwLoopPlan } from "./types.ts";
import { UlwLoopError } from "./types.ts";

export const ULW_LOOP_HELP = `Usage:
  lazy-claudecode ulw-loop create-goals --brief "..." [--brief-file <path>] [--from-stdin] [--claude-code-goal-mode aggregate|per_story] [--force] [--json]
  lazy-claudecode ulw-loop status [--json]
  lazy-claudecode ulw-loop complete-goals [--retry-failed] [--json]
  lazy-claudecode ulw-loop criteria --goal-id <id> [--json]
  lazy-claudecode ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status pass|fail|blocked --evidence "..." [--notes "..."] [--json]
  lazy-claudecode ulw-loop checkpoint --goal-id <id> --status complete|failed|blocked --evidence "..." --claude-code-goal-json <...> [--quality-gate-json <...>] [--json]
  lazy-claudecode ulw-loop steer --kind <kind> ... --evidence "..." --rationale "..." [--json]
  lazy-claudecode ulw-loop add-goal --title "..." --objective "..." [--json]
  lazy-claudecode ulw-loop record-review-blockers --goal-id <id> --title "..." --objective "..." --evidence "..." --claude-code-goal-json <...> [--json]

All subcommands accept [--session-id <id>] to isolate state under .claude/ulw-loop/<id>/; without it, Claude Code session env is used when present.`;
...
export function normalizeClaudeCodeGoalMode(value: string | undefined): UlwLoopClaudeCodeGoalMode {
	...
	throw new UlwLoopError(
		"Invalid --claude-code-goal-mode; expected aggregate or per_story.",
		"ULW_LOOP_CLAUDE_CODE_GOAL_MODE_INVALID",
		{ details: { value } },
	);
}
