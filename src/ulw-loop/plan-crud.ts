...
import { aggregateClaudeCodeObjectiveForScope, isUlwLoopDone } from "./goal-status.ts";
import {
	type UlwLoopScope,
	ulwLoopBriefPath,
...
function completedPlanExistsError(scope?: UlwLoopScope): UlwLoopError {
	return new UlwLoopError(
		[
			...
			"Start a new run with `lazy-claudecode ulw-loop create-goals --session-id <new-id> ...` to isolate fresh state.",
