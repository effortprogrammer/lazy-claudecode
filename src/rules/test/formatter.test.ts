...
import { formatDynamicBlock, formatStaticBlock } from "@effortprogrammer/rules-engine/engine";
import type { LoadedRule, MatchReason, RuleSource } from "@effortprogrammer/rules-engine/engine";
...
describe("rules formatter hook context", () => {
	it("#given multiline dynamic rules #when formatting PostToolUse context #then labels and bodies render on separate lines", () => {
		...
		const block = formatDynamicBlock(
			[rule],
			"packages/lazy-claudecode-claude-code/plugin/components/ulw-loop/src/paths.ts",
			FORMAT_OPTIONS,
		);
		...
		expect(block).toBe(
			[
				"Additional project instructions matched for packages/lazy-claudecode-claude-code/plugin/components/ulw-loop/src/paths.ts:",
				"",
