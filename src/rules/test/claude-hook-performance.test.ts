...
function makeTempProject(ruleCount: number): { root: string; pluginData: string; targetPath: string } {
	...
	for (let index = 0; index < ruleCount; index += 1) {
		fs.writeFileSync(
			path.join(root, ".claude", "rules", `rule-${index}.md`),
...
function isProjectRuleRead(filePath: unknown): boolean {
	return String(filePath).includes(`${path.sep}.claude${path.sep}rules${path.sep}`);
}

describe("claude-code rules hook performance", () => {
