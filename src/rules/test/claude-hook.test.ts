...
function makeTempProject(): { root: string; pluginData: string } {
	const root = mkdtempSync(path.join(tmpdir(), "claude-code-rules-project-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "claude-code-rules-data-"));
...
describe("claude-code rules hooks", () => {
	...
	it("#given default auto sources #when SessionStart runs #then native Claude Code AGENTS.md is not duplicated", async () => {
		// given
		const { root, pluginData } = makeTempProject();
	...
	it("#given project AGENTS.md #when SessionStart runs #then rules hook leaves AGENTS.md to Claude Code native handling", async () => {
		// given
		const { root, pluginData } = makeTempProject();
