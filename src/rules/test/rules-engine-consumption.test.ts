...
describe("rules-engine package consumption", () => {
	it("#given Claude Code bundled rule sources #when imported from rules-engine #then plugin-bundled ordering is preserved", () => {
		// given
	...
	it("#given Claude Code rule parser and matcher edge cases #when imported from rules-engine #then current behavior is preserved", () => {
		// given
		const malformedRule = "---\nglobs: [\n---\nBody";
	...
	it("#given Claude Code scanner options #when imported from rules-engine #then scan depth and max files remain bounded", () => {
		// given
		const root = mkdtempSync(join(tmpdir(), "claude-code-rules-scanner-"));
