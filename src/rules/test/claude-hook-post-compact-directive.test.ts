...
describe("claude-code rules post-compaction read directive", () => {
	it("#given injected rules dropped by compaction #when UserPromptSubmit recovers #then it lists rule paths in a mandatory read directive instead of bodies", async () => {
		...
		expect(context).toContain(".claude/rules/typescript.md");
...
function makeProject(): { root: string; pluginData: string } {
	const root = mkdtempSync(path.join(tmpdir(), "claude-code-rules-directive-project-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "claude-code-rules-directive-data-"));
