...
function ruleMarkdown(body: string): string {
	return [
		"---",
		"description: LAZY_CLAUDECODE Hephaestus baseline discipline for Claude Code",
		"alwaysApply: true",
		"---",
		"",
		body,
...
describe("plugin bundled rule priority", () => {
	...
	it("#given aggregate plugin root #when resolving rules root #then components rules directory is selected", () => {
		// given
		const aggregateRoot = mkdtempSync(join(tmpdir(), "claude-code-rules-aggregate-plugin-"));
