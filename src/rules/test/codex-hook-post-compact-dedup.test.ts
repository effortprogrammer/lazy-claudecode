...
describe("claude-code rules PostCompact deduplication", () => {
...
function makeTempProject(): { root: string; pluginData: string } {
	const root = mkdtempSync(path.join(tmpdir(), "claude-code-rules-compact-dedup-project-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "claude-code-rules-compact-dedup-data-"));
