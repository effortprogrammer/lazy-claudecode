...
function makeProject(): { projectRoot: string; homeRoot: string; targetPath: string } {
	const projectRoot = mkdtempSync(join(tmpdir(), "claude-code-rules-finder-project-"));
	const homeRoot = mkdtempSync(join(tmpdir(), "claude-code-rules-finder-home-"));
	tempDirectories.push(projectRoot, homeRoot);
	mkdirSync(join(projectRoot, "src", ".claude", "rules"), { recursive: true });
	...
	writeFileSync(join(projectRoot, "src", ".claude", "rules", "local.md"), "Local rule\n");
	writeFileSync(join(projectRoot, ".claude", "rules", "root.md"), "Root rule\n");
...
describe("findRuleCandidates", () => {
	it("#given project and user-home rules #when target file is inside project #then candidates keep source distance", () => {
		...
		expect(candidates.map(candidateSummary)).toEqual([
			".claude/rules:0:src/.claude/rules/local.md",
			".claude/rules:1:.claude/rules/root.md",
	...
	it("#given disabled source #when finding candidates #then matching source is omitted", () => {
		...
		const candidates = findRuleCandidates({
			...
			targetFile: targetPath,
			homeDir: homeRoot,
			disabledSources: new Set([".claude/rules", "~/.opencode/rules", "plugin-bundled"]),
		});
