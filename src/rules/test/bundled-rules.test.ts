...
function makeFixture(options: FixtureOptions = {}): Fixture {
	const root = mkdtempSync(join(tmpdir(), "claude-code-rules-bundled-project-"));
	const pluginRoot = mkdtempSync(join(tmpdir(), "claude-code-rules-bundled-plugin-"));
	const pluginData = mkdtempSync(join(tmpdir(), "claude-code-rules-bundled-data-"));
	...
	const projectRulePath = join(root, ".claude", "rules", "hephaestus.md");
...
describe("plugin bundled rules", () => {
	...
	it("#given bundled Hephaestus rule body exceeds per-rule cap #when SessionStart runs #then static context expands the body within result budget", async () => {
		// given
		const root = mkdtempSync(join(tmpdir(), "claude-code-rules-bundled-large-project-"));
		const pluginRoot = mkdtempSync(join(tmpdir(), "claude-code-rules-bundled-large-plugin-"));
		const pluginData = mkdtempSync(join(tmpdir(), "claude-code-rules-bundled-large-data-"));
		tempDirectories.push(root, pluginRoot, pluginData);
		writeFileSync(join(root, "package.json"), JSON.stringify({ name: "fixture" }));
		mkdirSync(join(pluginRoot, "bundled-rules"), { recursive: true });
	...
	it("#given project rule body exceeds per-rule cap #when SessionStart runs #then static context injects a truncated body", async () => {
		// given
		const root = mkdtempSync(join(tmpdir(), "claude-code-rules-project-large-project-"));
		const pluginRoot = mkdtempSync(join(tmpdir(), "claude-code-rules-project-large-plugin-"));
		const pluginData = mkdtempSync(join(tmpdir(), "claude-code-rules-project-large-data-"));
		tempDirectories.push(root, pluginRoot, pluginData);
		writeFileSync(join(root, "package.json"), JSON.stringify({ name: "fixture" }));
		mkdirSync(join(root, ".claude", "rules"), { recursive: true });
		mkdirSync(join(pluginRoot, "bundled-rules"), { recursive: true });
