...
import { runPostToolUseHook, runUserPromptSubmitHook } from "../src/claude-code-hook.js";
...
const PROMPT_CONTEXT_PRESSURE_CASES = [
	...
	[
		"#given Claude Code canonical context-window prompt and empty static cache #when UserPromptSubmit runs #then it emits no static context",
		[
			"error context_length_exceeded",
			"Claude Code ran out of room in the model's context window. Start a new thread before retrying.",
		].join("\n"),
	],
] as const;
const TRANSCRIPT_CONTEXT_PRESSURE_CASES = [
	...
	[
		"#given Claude Code canonical context-window transcript and empty static cache #when UserPromptSubmit runs #then it emits no static context",
		"#given Claude Code canonical context-window transcript and empty dynamic cache #when PostToolUse runs #then it emits no dynamic context",
...
describe("claude-code rules context-pressure recovery", () => {
...
function makeTempProject(): { readonly root: string; readonly pluginData: string } {
	const root = mkdtempSync(path.join(tmpdir(), "claude-code-rules-context-pressure-project-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "claude-code-rules-context-pressure-data-"));
	...
	writeFileSync(
		...
		[
			...
			JSON.stringify({
				...
				payload: {
					content:
						"Claude Code ran out of room in the model's context window. Start a new thread or clear earlier history before retrying.",
				},
			}),
			"",
		].join("\n"),
