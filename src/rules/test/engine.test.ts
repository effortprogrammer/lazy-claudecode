...
function makeCandidate(overrides: Partial<RuleCandidate> = {}): RuleCandidate {
	const candidate = {
		...
		realPath: join(projectRoot, ".claude", "rules", "typescript.md"),
		source: ".claude/rules",
		distance: 0,
		isGlobal: false,
		isSingleFile: false,
...
describe("rule engine default source selection", () => {
	it("#given auto source selection #when loading static rules #then Claude Code-native and Claude-home sources are disabled by default", () => {
		// given
...
describe("rule engine static loading", () => {
	...
	it("#given project candidate resolves outside project #when loading static rules #then the rule is skipped with a diagnostic", () => {
		// given
		const outsidePath = "/tmp/claude-code-rules-outside/.claude/rules/typescript.md";
