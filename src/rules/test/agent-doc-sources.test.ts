...
function makeAgentDocProject(): AgentDocProject {
	const root = mkdtempSync(path.join(tmpdir(), "claude-code-rules-agent-docs-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "claude-code-rules-agent-docs-data-"));
	...
	writeFileSync(
		...
		[
			...
			"---",
			"",
			"Dynamic .claude/rules context must not leak through removed-only allowlists.",
		].join("\n"),
	);
...
describe("agent doc sources", () => {
	for (const sourceList of REMOVED_AGENT_DOC_SOURCE_LISTS) {
		it(`#given ${sourceList} removed-only source allowlist #when SessionStart runs #then it emits no LAZY_CLAUDECODE rules context`, async () => {
			// given
	...
	it("#given nested AGENTS.md and removed-only source allowlist #when PostToolUse targets a child file #then it emits no dynamic LAZY_CLAUDECODE rules context", async () => {
		// given
