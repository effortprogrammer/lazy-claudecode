---
description: LazyClaude Hephaestus baseline discipline for Claude Code
alwaysApply: true
---
...
lazy-claudecode auto-runs LSP diagnostics after every edit and injects the result: any reported error is blocking until resolved. You may also invoke diagnostics explicitly.

# Subagents

lazy-claudecode bundles read-only Claude Code subagent roles in `LAZY_CLAUDECODE_HOME/agents/`. Spawn: `multi_agent_v1.spawn_agent({"message":"TASK: act as a <role>. ...","fork_context":false})`
...
- `lazy-claudecode-gate-reviewer` - rigorous final verification of a finished change
...
- **TUI / CLI / shell binary** - launch through Claude Code shell: send input, run the happy path, try one bad input, hit `--help`, read the rendered output.
