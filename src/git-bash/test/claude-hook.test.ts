...
describe("applyGitBashPreToolUseReminder", () => {
	it("#given first Windows Bash call #when hook runs #then emits non-blocking git_bash guidance", () => {
		...
		expect(parsed.hookSpecificOutput).toEqual({
			...
			additionalContext:
				"On Windows, prefer the LAZY_CLAUDECODE git_bash MCP for shell commands before using built-in exec_command. Use exec_command only when git_bash is unavailable or for non-shell operations.",
		});
	});
...
describe("runGitBashHookCli", () => {
	it("#given Claude Code PreToolUse stdin on Windows #when CLI hook runs #then it writes reminder JSON", async () => {
		// given
		const pluginDataRoot = createTemporaryDirectory("lazy-claudecode-git-bash-hook-");
