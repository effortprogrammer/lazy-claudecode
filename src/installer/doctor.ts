...
function check(
	name: string,
	fn: () => { status: "ok" | "warn" | "error"; message: string },
): CheckResult {
	try {
		const result = fn();
		return { name, ...result };
...
export async function doctor(root: string): Promise<void> {
	...
	results.push(
		check("Bun runtime", () => {
			try {
				...
				return {
					status: "error",
					message: "Not found. Install: curl -fsSL https://bun.sh/install | bash",
				};
			}
		}),
	);
	...
	results.push(
		check("Claude Code settings", () => {
			...
			if (!existsSync(settingsPath)) {
				return {
					status: "warn",
					message: `Not found: ${settingsPath}. Run: lazy-claudecode install`,
				};
			}
			try {
				...
				if (settings.hooks) {
					...
					if (raw.includes("lazy-claudecode") || raw.includes("lazy_claudecode")) {
						return {
							status: "ok",
							message: `${hookCount} hooks configured (lazy-claudecode detected)`,
						};
					}
					return {
						status: "warn",
						message: `${hookCount} hooks found but none from lazy-claudecode. Run: lazy-claudecode install`,
					};
				}
	...
	results.push(
		check("LAZY_CLAUDECODE_ROOT env var", () => {
			...
			return {
				status: "warn",
				message: `Not set. Add to shell profile: export LAZY_CLAUDECODE_ROOT="${root}"`,
			};
		}),
	);
	...
	if (hasErrors) {
		...
		console.log(
			"⚠️  Some checks have warnings. Everything may still work, but consider addressing them.",
		);
	} else {
