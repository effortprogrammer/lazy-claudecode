...
import { getClaudeSettingsPath, getStateDir } from "../utils/paths.js";
...
export async function doctor(root: string): Promise<void> {
  ...
  results.push(
    check("Built files (dist/)", () => {
      const distPath = join(root, "dist");
      if (existsSync(distPath)) {
        return { status: "ok", message: "dist/ exists" };
      }
      return { status: "error", message: "Not found. Run: npm run build" };
    })
  );
