...
function resolveBunPath(): string {
	...
	const brewPaths = ["/opt/homebrew/bin/bun", "/usr/local/bin/bun"];
...
function loadHooksConfig(root: string): HooksConfig {
	...
	const resolved = raw.replace(/\$\{BUN\}/g, bunPath).replace(/\$\{PLUGIN_ROOT\}/g, root);
...
function installCrossCallAgents(root: string): number {
	...
	const deprecatedFiles = ["claude-code-delegate.md", "codex-delegate.md"];
	...
	for (const { file, requiredCli, cliName } of agentMapping) {
		...
		console.log(
			`✅ Installed ${file} → ${claudeAgentsDir}/ (${cliName} can delegate to ${requiredCli === "codex" ? "Claude Code" : "Codex"})`,
		);
		installed++;
	}
...
export async function install(root: string): Promise<void> {
	...
	console.log(
		`✅ Installation complete! ${hookCount} hooks installed across ${Object.keys(settings.hooks).length} events.`,
	);
...
async function installCompanionPlugins(): Promise<void> {
	...
	if (!hasClaude) {
		...
		console.log(
			"   Install manually: claude plugin marketplace add effortprogrammer/lazycc-codex-plugin && claude plugin install codex@lazycc-codex",
		);
		return;
	}
