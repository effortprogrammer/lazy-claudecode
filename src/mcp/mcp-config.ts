	...
	return {
		mcpServers: {
			codegraph: {
				...
				description:
					"Codebase structure analysis — understand code architecture, dependencies, and navigation",
			},
			context7: {
				command: "npx",
...
export function writeMcpConfig(projectDir: string): void {
	...
	if (existsSync(mcpPath)) {
		try {
			const existing = JSON.parse(require("node:fs").readFileSync(mcpPath, "utf-8")) as McpConfig;
