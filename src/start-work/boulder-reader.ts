...
const SESSION_ID_PREFIX_PATTERN = /^(claude-code|opencode):/;
...
export function readContinuationState(cwd: string, sessionId: string): ContinuationState | null {
	...
	const work = getWorkForSession(boulderState, normalizeSessionId(sessionId, "claude-code"));
	...
	return {
		...
		planPath,
		boulderPath,
		ledgerPath: join(cwd, ".claude", "start-work", "ledger.jsonl"),
...
function getBoulderFilePath(cwd: string): string {
	return join(cwd, ".claude", "boulder.json");
}
