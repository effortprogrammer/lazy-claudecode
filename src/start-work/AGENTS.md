...
- `hooks/hooks.json` registers Claude Code `Stop` and `SubagentStop` hooks.
...
- Never let the hook block a Claude Code turn because of malformed input.
- Never make a network call from the hook.
- Keep the directive in `directive.md`. Do not inline it into TypeScript files.
- The hook only continues sessions listed in `.lazy-claudecode/boulder.json` as `claude-code:<session_id>`.
