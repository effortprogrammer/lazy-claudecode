...
describe("start-work boulder state reader", () => {
	it("#given active claude-code work with remaining checklist #when state is read #then continuation fields match baseline", () => {
		// given
		const workspace = createWorkspace({
			boulderJson: createBoulderJson({ status: "active", sessionIds: ["claude-code:sess_abc"] }),
			planMarkdown: "# Plan\n\n## TODOs\n- [ ] First\n- [x] Done\n- [ ] Second\n",
		});
		...
		expect(state).toEqual({
			...
			planPath: join(workspace, ".claude", "plans", "plan.md"),
			boulderPath: join(workspace, ".claude", "boulder.json"),
			ledgerPath: join(workspace, ".claude", "start-work", "ledger.jsonl"),
			worktreePath: null,
			checklist: { completed: 1, remaining: 2, total: 3, nextTaskLabel: "First" },
		});
	});

	it("#given completed claude-code work #when state is read #then continuation is absent", () => {
		// given
		const workspace = createWorkspace({
			boulderJson: createBoulderJson({ status: "completed", sessionIds: ["claude-code:sess_abc"] }),
			planMarkdown: "# Plan\n\n## TODOs\n- [ ] First\n",
		});
	...
	it("#given paused claude-code work with remaining checklist #when state is read #then continuation is present", () => {
		// given
		const workspace = createWorkspace({
			boulderJson: createBoulderJson({ status: "paused", sessionIds: ["claude-code:sess_abc"] }),
			planMarkdown: "# Plan\n\n## TODOs\n- [ ] First\n",
		});
	...
	it("#given bare boulder session id #when claude-code state is read #then continuation is absent", () => {
		// given
		const workspace = createWorkspace({
	...
	it("#given works omit the claude-code session but stale mirror matches #when state is read #then continuation is absent", () => {
		// given
		const workspace = createWorkspace({
			boulderJson: JSON.stringify({
				...
				session_ids: ["claude-code:sess_abc"],
			}),
			planMarkdown: "# Plan\n\n## TODOs\n- [ ] First\n",
		});
...
function createPlan(markdown: string): string {
	const root = mkdtempSync(join(tmpdir(), "claude-code-continuation-plan-"));
...
function createWorkspace(input: WorkspaceInput): string {
	const root = mkdtempSync(join(tmpdir(), "claude-code-continuation-reader-"));
	cleanupRoots.push(root);
	mkdirSync(join(root, ".claude", "plans"), { recursive: true });
	writeFileSync(join(root, ".claude", "plans", "plan.md"), input.planMarkdown);
	writeFileSync(join(root, ".claude", "boulder.json"), input.boulderJson);
	return root;
}
