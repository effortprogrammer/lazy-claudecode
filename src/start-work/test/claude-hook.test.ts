...
import { runStopHook } from "../src/claude-code-hook.js";
...
describe("start-work Stop hook", () => {
	...
	it("#given active claude-code work with remaining top-level tasks #when hook runs #then returns block JSON", () => {
		// given
		const workspace = createWorkspace({
			boulderJson: createBoulderJson({
				sessionIds: ["claude-code:sess_abc"],
				status: "active",
	...
	it("#given active claude-code work #when continuation directive is emitted #then subagent guidance is reliable", () => {
		// given
		const workspace = createWorkspace({
			boulderJson: createBoulderJson({ sessionIds: ["claude-code:sess_abc"], status: "active" }),
	...
	it("#given active claude-code work #when continuation directive is emitted #then QA weight is tier-scoped without echo bloat", () => {
		// given
		const workspace = createWorkspace({
			boulderJson: createBoulderJson({ sessionIds: ["claude-code:sess_abc"], status: "active" }),
			planMarkdown: ["# Plan", "", "## TODOs", "- [ ] First"].join("\n"),
	...
	it("#given completed boulder work #when hook runs #then returns empty output", () => {
		// given
		const workspace = createWorkspace({
			boulderJson: createBoulderJson({ sessionIds: ["claude-code:sess_abc"], status: "completed" }),
			planMarkdown: "- [ ] First",
		});
		const fs = createMemoryFs();
...
function createWorkspace(input: WorkspaceInput): string {
	const root = mkdtempSync(join(tmpdir(), "claude-code-continuation-hook-"));
