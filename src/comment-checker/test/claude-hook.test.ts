...
describe("extractCodexCommentCheckRequests", () => {
	it("#given claude-code apply_patch command #when extracting #then returns edit request for changed file", () => {
	...
	it("#given claude-code write payload #when extracting #then returns write request", () => {
		const requests = extractCodexCommentCheckRequests(
			postToolUseInput({
				tool_name: "write",
				tool_input: {
	...
	it("#given claude-code edit payload #when extracting #then returns edit request", () => {
		const requests = extractCodexCommentCheckRequests(
			postToolUseInput({
				tool_name: "edit",
				tool_input: {
	...
	it("#given one-sided claude-code edit payload #when extracting #then returns no requests", () => {
		const requests = extractCodexCommentCheckRequests(
			postToolUseInput({
				tool_name: "edit",
				tool_input: {
	...
	it("#given claude-code multi_edit payload #when extracting #then returns multiedit request", () => {
		const requests = extractCodexCommentCheckRequests(
			postToolUseInput({
...
describe("runCommentCheckerPostToolUse", () => {
	...
	it("#given transcript path #when hook runs #then forwards it to checker input", async () => {
		...
		await runCommentCheckerPostToolUse(
			postToolUseInput({
				transcript_path: "/tmp/claude-code-comment-checker-transcript.jsonl",
				tool_name: "write",
				tool_input: {
					file_path: "src/example.ts",
					content: "// explains value\nconst value = 1;\n",
		...
		expect(transcriptPath).toBe("/tmp/claude-code-comment-checker-transcript.jsonl");
	});
	...
	it("#given Claude Code canonical context-window transcript and long checker warning #when hook blocks #then it caps feedback", async () => {
		const root = mkdtempSync(path.join(tmpdir(), "claude-code-comment-checker-context-pressure-"));
		tempDirs.push(root);
