...
- `node dist/cli.js hook post-tool-use < fixture.json` - smoke-test the Claude Code hook.

## Constraints

- No Bun APIs. Runtime is Node only because Claude Code launches plugin hooks with Node.
- Keep Claude Code `PostToolUse` hook behavior covered by tests.
- Keep `apply_patch` extraction covered by tests.
- `apply_patch` must support Claude Code `tool_input.command`, raw patch text, and OMO-compatible metadata.
- Hook output must use the stable Claude Code hook JSON contract.
...
- Do not couple this package back to pi, lazy-claudecode, or senpi internal source paths.
