...
| `write`, `edit`, `multi_edit`, or `multiedit` succeeds | maps the Claude Code payload to the native checker hook input |
| non-edit tool succeeds | ignored |
| checker exits `2` | returns Claude Code `PostToolUse` blocking feedback so the model fixes or explains the warning |
...
- [pi-comment-checker](https://github.com/code-yeongyu/pi-comment-checker) - source extension this Claude Code plugin ports.
