...
import {
	// TODO: re-enable when API is ported
...
// though Claude Code deletes old version dirs on upgrade (core-plugins store.rs).
...
// degraded mode there: record the lazy-claudecode-cli ledger entry and log the same
// warning install-local.mjs prints instead of leaving a broken link.
async function linkRuntimeWrapperStep(
	...
	try {
		...
		degraded.push({
			component: "lazy-claudecode-cli",
			hint: BOOTSTRAP_DOCTOR_HINT,
			reason: `failed to link the lazy-claudecode runtime wrapper into ${binDir}: ${errorMessage(error)}`,
		});
	}
}
