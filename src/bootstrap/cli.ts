...
const TOP_LEVEL_HELP =
	"Usage:\n  lazy-claudecode-bootstrap hook session-start\n  lazy-claudecode-bootstrap worker [--claude-code-home <dir>] [--once] [--only <step>] [--manifest-dir <dir>]\n  lazy-claudecode-bootstrap download <manifest> <platform> <destination-dir>\n  lazy-claudecode-bootstrap help | --help | -h\n";
...
async function runWorkerCommand(args: readonly string[]): Promise<number> {
	...
	try {
		...
		if (/flag/.test(message)) {
			process.stderr.write(`[lazy-claudecode-bootstrap] ${message}\n${TOP_LEVEL_HELP}`);
			return 1;
		}
		// Runtime worker failures must never surface as non-zero exits; the
		// degraded ledger in state.json is the error channel.
		process.stderr.write(`[lazy-claudecode-bootstrap] worker error: ${message}\n`);
		return 0;
	}
