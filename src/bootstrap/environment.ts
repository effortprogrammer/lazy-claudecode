...
export const INSTALL_SNAPSHOT_FILENAME = "lazyclaude-code-install.json";
...
export async function resolveClaudeCodeHome(
	...
	if (options.pluginRoot !== undefined) {
		...
		for (let level = 0; level < MAX_CLAUDE_CODE_HOME_WALK_UP_LEVELS; level += 1) {
	...
	return { path: join(homedir(), ".claude-code"), source: "default" };
}
...
export async function bootstrapLocks(
	...
	const autoUpdateLockPath = resolveLockPath(resolveStatePath("auto-update"));

	let bootstrapRelease: (() => Promise<void>) | null;
	try {
		bootstrapRelease = await acquireLock(bootstrapLockPath, staleMs);
	} catch {
		return null;
	}
	...
	try {
		autoUpdateRelease = await acquireLock(autoUpdateLockPath, staleMs);
	} catch {
		await bootstrapRelease();
		return null;
	}
