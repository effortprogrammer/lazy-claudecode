...
const CREATE_THREAD_TOOL_NAMES = new Set(["create_thread", "claude-code_app.create_thread"]);
...
function threadTitleReminder(threadReference: ThreadCreationReference): string {
	...
	return threadReference.kind === "thread"
		? `THREAD ID ${id}: CALL claude-code_app.set_thread_title NOW. USE THE REAL TASK/ROLE.`
		: `PENDING WORKTREE ID ${id}: CALL claude-code_app.set_thread_title AS SOON AS THREAD ID EXISTS. USE THE REAL TASK/ROLE.`;
}
