...
describe("CodeGraph SessionStart hook", () => {
	it("#given hook session-start cli args #when invoked with empty JSON input #then it emits valid JSON and exits zero", async () => {
		...
		const homeDir = mkdtempSync(join(tmpdir(), "lazy-claudecode-codegraph-hook-home-"));
		try {
			// when
			const exitCode = await runCodegraphCli({
				argv: ["node", "cli.js", "hook", "session-start"],
				cwd: mkdtempSync(join(tmpdir(), "lazy-claudecode-codegraph-hook-workspace-")),
				env: { HOME: homeDir },
				stdin: Readable.from(["{}"]),
	...
	it("#given CodeGraph MCP reports an uninitialized project #when PostToolUse fires #then it emits LAZY_CLAUDECODE global-store init guidance", async () => {
		// given
	...
	it("#given HOME LAZY_CLAUDECODE config disables Claude Code CodeGraph #when SessionStart fires #then it skips without spawning", async () => {
		// given
		const homeDir = mkdtempSync(join(tmpdir(), "lazy-claudecode-codegraph-sot-home-"));
		const workspace = mkdtempSync(join(tmpdir(), "lazy-claudecode-codegraph-sot-workspace-"));
		const stdout: string[] = [];
		const spawned: WorkerSpawnInvocation[] = [];
		try {
			...
			writeFileSync(
				...
				'{ "codegraph": { "enabled": true }, "[claude-code]": { "codegraph": { "enabled": false } } }\n',
			);
	...
	it("#given env disables CodeGraph over SOT enablement #when SessionStart fires #then it skips without spawning", async () => {
		// given
		const homeDir = mkdtempSync(join(tmpdir(), "lazy-claudecode-codegraph-env-home-"));
		const stdout: string[] = [];
		const spawned: WorkerSpawnInvocation[] = [];
		try {
			mkdirSync(join(homeDir, ".claude"), { recursive: true });
	...
	it("#given CodeGraph is enabled #when SessionStart fires #then it detaches a background worker immediately", async () => {
		...
		const stdout: string[] = [];
		const spawned: WorkerSpawnInvocation[] = [];
		const workspace = mkdtempSync(join(tmpdir(), "lazy-claudecode-codegraph-workspace-"));
