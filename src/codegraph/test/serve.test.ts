...
describe("runCodegraphServe", () => {
	it("#given CodeGraph resolves #when serving MCP #then execs codegraph serve --mcp with inherited stdio and telemetry disabled", async () => {
		...
		const exitCode = await runCodegraphServe({
			...
			homeDir: "/tmp/home",
			buildEnv: ({ homeDir }) => ({
				CODEGRAPH_INSTALL_DIR: `${homeDir}/.claude/codegraph`,
		...
		expect(calls).toEqual([
			{
				...
				command: "node",
				env: {
					CODEGRAPH_INSTALL_DIR: "/tmp/home/.claude/codegraph",
					CODEGRAPH_NO_DOWNLOAD: "1",
					CODEGRAPH_TELEMETRY: "0",
					CUSTOM: "keep",
					DO_NOT_TRACK: "1",
	...
	it("#given Windows Claude Code SOT install_dir has codegraph.cmd #when serving MCP #then it resolves there and exports CODEGRAPH_INSTALL_DIR", async () => {
		await withProcessPlatform("win32", async () => {
			// given
			const tempRoot = mkdtempSync(join(tmpdir(), "lazy-claudecode-codegraph-serve-install-dir-"));
	...
	it("#given Windows CodeGraph resolves to a cmd shim #when resolving serve invocation #then cmd.exe executes the shim", () => {
		// given
		const shimPath = "C:\\Users\\runner\\.claude\\codegraph\\bin\\codegraph.cmd";
...
function createFakeCodegraphRoot(): string {
	const tempRoot = mkdtempSync(join(tmpdir(), "lazy-claudecode-codegraph-wrapper-"));
