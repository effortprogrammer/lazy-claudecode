...
const CODEGRAPH_DISABLED_HINT =
	"CodeGraph MCP skipped: disabled by OMO SOT config. Set [claude-code].codegraph.enabled=true to enable it.\n";
...
async function provisionMissingCodegraph(options: {
	...
	const installDir = options.trustedInstallDir ?? join(options.homeDir, ".claude", "codegraph");
