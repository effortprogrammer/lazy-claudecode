...
function defaultProvisionedBin(homeDir, fileExists) {
  ...
  const candidates = [
    ...
    join4(homeDir, ".lazy-claudecode", "codegraph", "node-servers", "node_modules", ".bin", binaryName)
  ];
...
async function provisionMissingCodegraph(options) {
  ...
  if (options.config.auto_provision === false)
    return null;
  const installDir = options.trustedInstallDir ?? join6(options.homeDir, ".lazy-claudecode", "codegraph");
