...
function buildCodegraphEnv(options = {}) {
	...
	return {
		[CODEGRAPH_INSTALL_DIR_ENV]: join(homeDir, ".lazy-claudecode", "codegraph"),
...
function defaultInstallDir() {
	return join2(homedir2(), ".lazy-claudecode", "codegraph");
}
...
async function installAsset(layout) {
	...
	try {
		...
		if (actualChecksum !== asset.sha256) {
			throw new Error(
				`checksum mismatch for ${basename(asset.url)}: expected ${asset.sha256}, got ${actualChecksum}`,
			);
		}
...
function defaultProvisionedBin(homeDir, fileExists) {
	...
	const candidates = [
		join4(homeDir, ".lazy-claudecode", "codegraph", "bin", binaryName),
		join4(
			homeDir,
			".lazy-claudecode",
			"codegraph",
...
function resolveBundledShim(requireResolve, fileExists) {
	try {
		...
		const candidates = [
			join4(packageRoot, "bin", "codegraph.js"),
			join4(packageRoot, "npm-shim.js"),
		];
		return candidates.find((candidate) => fileExists(candidate)) ?? null;
	} catch (error) {
		if (error instanceof Error) return null;
...
function resolveCodegraphCommand(options = {}) {
	...
	const provisioned =
		options.provisioned?.() ?? defaultProvisionedBin(options.homeDir ?? homedir3(), fileExists);
...
function findProjectConfigPathsNearestFirst(cwd, homeDir) {
	...
	while (true) {
		...
		const configPath = join5(currentDir, ".lazy-claudecode", "config.jsonc");
...
function resolveOmoConfigPaths(options) {
	const globalPath = join5(resolve(options.homeDir), ".lazy-claudecode", "config.jsonc");
...
async function runCodegraphServe(options = {}) {
	...
	const config =
		options.config ?? getCodexOmoConfig({ cwd: options.cwd ?? processCwd(), env, homeDir });
	...
	if (
		!resolution.exists ||
		shouldSkipResolvedCommand(resolution, options.commandExists ?? existsSync5)
	) {
...
async function provisionMissingCodegraph(options) {
	...
	const installDir =
		options.trustedInstallDir ?? join6(options.homeDir, ".lazy-claudecode", "codegraph");
...
export { runCodegraphServeCli, runCodegraphServe, resolveServeProcessInvocation };
