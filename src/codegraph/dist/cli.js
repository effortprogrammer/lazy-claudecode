...
var CODEGRAPH_ENV_BIN = "LAZY_CLAUDECODE_CODEGRAPH_BIN";
...
function defaultProvisionedBin(homeDir, fileExists) {
  ...
  const candidates = [
    ...
    join6(homeDir, ".lazy-claudecode", "codegraph", "node-servers", "node_modules", ".bin", binaryName)
  ];
...
// components/codegraph/src/session-start-worker.ts
var SESSION_START_CWD_ENV = "LAZY_CLAUDECODE_CODEGRAPH_SESSION_START_CWD";
...
// components/codegraph/src/serve.ts
var CODEGRAPH_SKIP_HINT = `CodeGraph MCP skipped: codegraph binary not found. Install CodeGraph or set LAZY_CLAUDECODE_CODEGRAPH_BIN.
`;
...
async function provisionMissingCodegraph(options) {
  ...
  if (options.config.auto_provision === false)
    return null;
  const installDir = options.trustedInstallDir ?? join8(options.homeDir, ".lazy-claudecode", "codegraph");
