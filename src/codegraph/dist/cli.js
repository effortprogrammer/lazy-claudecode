...
function codegraphDataRoot(homeDir) {
	return join(homeDir, ".lazy-claudecode", "codegraph");
}
...
function looksLikeCodegraphUninitializedOutput(output) {
	...
	return (
		CODEGRAPH_STATUS_UNINITIALIZED_PATTERN.test(normalizedOutput) &&
...
function visit(text, visitor, options = ParseOptions.DEFAULT) {
	...
	const onObjectEnd = toEndVisit(visitor.onObjectEnd);
	const onArrayBegin = toBeginVisit(visitor.onArrayBegin);
	const onArrayEnd = toEndVisit(visitor.onArrayEnd);
...
const HARNESS_IDS = ["codex", "opencode", "omo"];
const SETTING_HARNESS_SUPPORT = {
...
function findProjectConfigPathsNearestFirst(cwd, homeDir) {
	...
	while (true) {
		...
		const configPath = join2(currentDir, ".lazy-claudecode", "config.jsonc");
...
function resolveOmoConfigPaths(options) {
	const globalPath = join2(resolve2(options.homeDir), ".lazy-claudecode", "config.jsonc");
	const projectPathsFarthestFirst = findProjectConfigPathsNearestFirst(
		options.cwd,
		options.homeDir,
	).reverse();
	return [
		...
		if (parsed.errors.length > 0) {
			return {
				...
				warnings: parsed.errors.map(
					(error) => `JSONC parse error in ${path}: ${error.message} at offset ${error.offset}`,
				),
			};
		}
		const warnings = [];
...
const CODEGRAPH_INSTALL_DIR_ENV = "CODEGRAPH_INSTALL_DIR";
const CODEGRAPH_NO_DOWNLOAD_ENV = "CODEGRAPH_NO_DOWNLOAD";
const CODEGRAPH_TELEMETRY_ENV = "CODEGRAPH_TELEMETRY";
const DO_NOT_TRACK_ENV = "DO_NOT_TRACK";
function buildCodegraphEnv(options = {}) {
	const homeDir = options.homeDir ?? homedir5();
	return {
		[CODEGRAPH_INSTALL_DIR_ENV]: join3(homeDir, ".lazy-claudecode", "codegraph"),
...
const CODEGRAPH_MIN_NODE_MAJOR = 20;
const CODEGRAPH_BLOCKED_NODE_MAJOR = 25;
const CODEGRAPH_UNSAFE_NODE_ENV = "CODEGRAPH_ALLOW_UNSAFE_NODE";
const CODEGRAPH_NODE_BIN_ENV = "CODEGRAPH_NODE_BIN";
...
const CODEGRAPH_PROVISION_MANIFEST = {
	assets: {
		"darwin-arm64": {
...
function defaultInstallDir() {
	return join4(homedir6(), ".lazy-claudecode", "codegraph");
}
...
async function installAsset(layout) {
	...
	try {
		...
		if (actualChecksum !== asset.sha256) {
			throw new Error(
				`checksum mismatch for ${basename2(asset.url)}: expected ${asset.sha256}, got ${actualChecksum}`,
			);
		}
...
function isUnsafeCommandName(commandName) {
	...
	if (commandName.includes("\x00")) return true;
	return false;
}
...
const CODEGRAPH_ENV_BIN = "LAZY_CLAUDECODE_CODEGRAPH_BIN";
const CODEGRAPH_LEGACY_ENV_BIN = "CODEGRAPH_BIN";
const CODEGRAPH_NODE_CANDIDATES = ["node24", "node22", "node20", "node"];
...
function defaultNodeRuntime(env, fileExists, which, nodeVersion) {
	...
	if (configured !== undefined && configured.length > 0) {
		...
		return resolved !== null && supportsCodegraphNodeRuntime(resolved, env, nodeVersion)
			? resolved
			: null;
	}
	const candidates = [
		...(isNodeExecutableName(process.execPath) ? [process.execPath] : []),
		...CODEGRAPH_NODE_CANDIDATES.map((commandName) => which(commandName)).filter(
...
function defaultProvisionedBin(homeDir, fileExists) {
	...
	const candidates = [
		join6(homeDir, ".lazy-claudecode", "codegraph", "bin", binaryName),
		join6(
			homeDir,
			".lazy-claudecode",
			"codegraph",
...
function resolveBundledShim(requireResolve, fileExists) {
	try {
		...
		const candidates = [
			join6(packageRoot, "bin", "codegraph.js"),
			join6(packageRoot, "npm-shim.js"),
		];
		return candidates.find((candidate) => fileExists(candidate)) ?? null;
	} catch (error) {
		if (error instanceof Error) return null;
...
function resolveCodegraphCommand(options = {}) {
	...
	const nodeRuntime =
		options.nodeRuntime ??
		(() => defaultNodeRuntime(env, fileExists, which, options.nodeVersion ?? defaultNodeVersion));
	...
	const provisioned =
		options.provisioned?.() ?? defaultProvisionedBin(options.homeDir ?? homedir7(), fileExists);
...
// components/codegraph/src/session-start-worker.ts
const SESSION_START_CWD_ENV = "LAZY_CLAUDECODE_CODEGRAPH_SESSION_START_CWD";
...
async function runBootstrap(projectRoot, config, env, homeDir, nodeSupport, deps, logOutcome) {
	try {
		...
		if (command.kind === "unavailable") {
			return finish(
				"skipped-unavailable",
				{ error: command.error, projectRoot, source: command.source },
				logOutcome,
			);
		}
		...
		const status = await deps.runCommand(
			...
			[...command.resolution.argsPrefix, "status", "--json"],
			{ env: codegraphEnv, timeoutMs: COMMAND_TIMEOUT_MS },
		);
		const decision = decideStartupAction(status);
		if (decision.kind === "skip")
			return finish("skipped-status", { error: decision.reason, projectRoot }, logOutcome);
		const actionArgs = command.resolution.argsPrefix.concat(
			decision.kind === "init" ? ["init"] : ["sync"],
		);
		const action = await deps.runCommand(projectRoot, command.resolution.command, actionArgs, {
			env: codegraphEnv,
			timeoutMs: COMMAND_TIMEOUT_MS,
		});
		return finish(
			decision.kind === "init" ? "initialized" : "synced",
			{
		...
		return finish(
			"failed",
			{ error: error instanceof Error ? error.message : String(error), projectRoot },
			logOutcome,
		);
	}
}
...
async function resolveOrProvisionCommand(deps, config, env, homeDir, nodeSupport) {
	...
	const resolved = deps.resolveCommand({
		env,
		homeDir,
	...
	const installDir = trustedInstallDir ?? join7(homeDir, ".lazy-claudecode", "codegraph");
...
function appendOutcome(homeDir, outcome) {
	const logDir = join7(homeDir, ".lazy-claudecode", "codegraph");
	mkdirSync2(logDir, { recursive: true });
	appendFileSync2(
		join7(logDir, "session-start.jsonl"),
...
// components/codegraph/src/hook.ts
const CODEGRAPH_SESSION_START_NOTICE = "LazyCodex CodeGraph bootstrap scheduled in background";
...
function runCodegraphPostToolUseHook(input, options = {}) {
	...
	const toolOutput = isRecord4(input)
...
function resolveProjectRoot(input, fallback) {
	if (!isRecord4(input)) return fallback;
	const cwd = input.cwd;
...
const HEADER_SEPARATOR = Buffer.from(`\r
\r
`);
...
const DEFAULT_IDLE_TIMEOUT_MS = 10 * 60000;
const noopLog = () => {};
...
// components/codegraph/src/serve.ts
const CODEGRAPH_SKIP_HINT = `CodeGraph MCP skipped: codegraph binary not found. Install CodeGraph or set LAZY_CLAUDECODE_CODEGRAPH_BIN.
`;
const CODEGRAPH_DISABLED_HINT = `CodeGraph MCP skipped: disabled by OMO SOT config. Set [codex].codegraph.enabled=true to enable it.
`;
const CODEGRAPH_VERSION2 = "1.0.1";
const WINDOWS_CMD_EXTENSIONS2 = new Set([".bat", ".cmd"]);
...
async function runCodegraphServe(options = {}) {
	...
	const config =
		options.config ?? getCodexOmoConfig({ cwd: options.cwd ?? processCwd3(), env, homeDir });
	...
	if (
		!resolution.exists ||
		shouldSkipResolvedCommand(resolution, options.commandExists ?? existsSync7)
	) {
...
async function provisionMissingCodegraph(options) {
	...
	const installDir =
		options.trustedInstallDir ?? join8(options.homeDir, ".lazy-claudecode", "codegraph");
...
function shouldSkipResolvedCommand(resolution, commandExists) {
	...
	if (!looksLikePath2(resolution.command)) return false;
