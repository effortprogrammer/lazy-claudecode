...
stderr tail: ${stderrTail}`
			: "";
		super(
			`LSP server ${serverId} at ${root} exited with code ${exitCode ?? "null"}${stderrSuffix}`,
		);
		this.serverId = serverId;
		this.root = root;
...
function reportBestEffortCleanupError(operation, error) {
	if (process.env.LAZY_CLAUDECODE_LSP_DEBUG_CLEANUP !== "1") return;
...
const STOP_HARD_KILL_TIMEOUT_MS = 5000;
const STOP_SIGKILL_GRACE_MS = 1000;

// ../lsp-core/src/lsp/json-rpc-connection.ts
const HEADER_SEPARATOR2 = `\r
\r
`;
...
class JsonRpcConnection {
	...
	dispatchBody(body) {
		...
		try {
			...
			).catch((writeError) => this.emitError(toError(writeError)));
			return;
		}
		if (!isJsonRpcObject(parsed)) {
			this.writeError(null, INVALID_REQUEST, "Invalid JSON-RPC message").catch((error) =>
		...
		if (typeof parsed.method !== "string") {
			...
			this.writeError(id, INVALID_REQUEST, "Invalid JSON-RPC method").catch((error) =>
				this.emitError(toError(error)),
			);
			return;
		}
	...
	handleRequest(message) {
		const id = getMessageId(message);
		if (id === undefined) {
			this.writeError(null, INVALID_REQUEST, "Invalid JSON-RPC id").catch((error) =>
				this.emitError(toError(error)),
			);
			return;
		}
		...
		if (!handler) {
			this.writeError(id, METHOD_NOT_FOUND, `Method not found: ${method}`).catch((error) =>
				this.emitError(toError(error)),
			);
			return;
		}
...
function getInstallDecisionsPath() {
	...
	if (!override) return join2(homedir(), ".codex", "lsp-install-decisions.json");
...
function getUserConfigPath() {
	...
	if (!userOverride) return join3(homedir2(), ".codex", "lsp-client.json");
...
function isLspEntry(value) {
	...
	return (
		...
		(initialization === undefined || isRecord3(initialization))
	);
}
...
function isServerInstalled(command, _workingDirectory) {
	...
	if (cmd.includes("/") || cmd.includes("\\")) {
		if (existsSync4(cmd)) return true;
	}
...
function findServerForExtension(ext) {
			...
			if (server2.env !== undefined) {
				return {
					...
					server: {
						...resolvedServer,
						env: server2.env,
						...(server2.initialization === undefined
							? {}
							: { initialization: server2.initialization }),
					},
				};
			}
			return {
	...
	for (const server2 of servers) {
		if (server2.extensions.includes(ext)) {
			const installHint =
				LSP_INSTALL_HINTS[server2.id] ??
				`Install '${server2.command[0]}' and ensure it's in your PATH`;
			return {
...
function getAllServers() {
	...
	const seen = new Set();
	for (const server2 of servers) {
		if (seen.has(server2.id)) continue;
		result.push({
			id: server2.id,
...
const SKIP_DIRECTORIES2 = new Set(["node_modules", ".git", "dist", "build", ".next", "out"]);
...
function handleMissingDependencyError(error) {
	...
	return message.includes("NOT INSTALLED") || message.includes("No LSP server configured")
		? message
		: null;
}
...
function realpathForValidation(filePath) {
	if (existsSync7(filePath)) return realpathSync(filePath);
...
`);
		...
		for (const edit of sortedEdits) {
			...
			if (startLine === endLine) {
				...
				const newContent =
					firstLine.substring(0, startChar) + edit.newText + lastLine.substring(endChar);
				lines.splice(
					startLine,
					endLine - startLine + 1,
					...newContent.split(`
...
async function executeLspSymbols(params, signal) {
	...
	try {
		if (scope === "workspace") {
			...
			const symbols2 = await withLspClient(
				filePath,
...
function defaultEnsureDaemonDeps() {
	return {
		...
		cleanupStaleSocket: (socketPath) => {
			if (existsSync8(socketPath)) unlinkQuietly(socketPath);
		},
...
function daemonBaseDir(env = process.env) {
	const explicit = env.LAZY_CLAUDECODE_LSP_DAEMON_DIR?.trim();
	if (explicit) return explicit;
	const pluginData = env.PLUGIN_DATA?.trim();
	if (pluginData) return join8(pluginData, "daemon");
	const codexHome = env.LAZY_CLAUDECODE_HOME?.trim();
...
function extractRequestContext(raw) {
	if (!isRecord5(raw) || raw.method !== "tools/call") return { input: raw, context: undefined };
	const params = raw.params;
	if (!isRecord5(params)) return { input: raw, context: undefined };
	const args = params.arguments;
	if (!isRecord5(args)) return { input: raw, context: undefined };
...
function parseContext(value) {
	...
	const env = value.env;
	if (isStringRecord2(env)) context.env = env;
...
function sendToolCall(socketPath, name, args, timeoutMs) {
	return new Promise((resolve6, reject) => {
		...
		const decoder = createLineDecoder((message) => {
			...
			if (result) finish(() => resolve6(result));
		...
		socket.once("error", (error) =>
			finish(() => reject(new DaemonRequestError(error.message, requestWritten))),
		);
		socket.once("close", () =>
...
function toToolResult(message) {
	if (!isRecord6(message) || message.id !== REQUEST_ID) return null;
	const result = message.result;
	if (!isRecord6(result) || !Array.isArray(result.content)) return null;
	return {
		content: result.content,
...
const DEFAULT_IDLE_SHUTDOWN_MS = 30 * 60000;
const DEFAULT_IDLE_CHECK_INTERVAL_MS = 60000;
