...
function defaultEnsureDaemonDeps() {
	return {
		...
		cleanupStaleSocket: (socketPath) => {
			if (existsSync2(socketPath)) unlinkQuietly(socketPath);
		},
...
function daemonBaseDir(env = process.env) {
	...
	if (pluginData) return join2(pluginData, "daemon");
...
const HEADER_SEPARATOR2 = Buffer.from(`\r
\r
`);
...
function getInstallDecisionsPath() {
	...
	if (!override) return join3(homedir2(), ".codex", "lsp-install-decisions.json");
...
function getUserConfigPath() {
	...
	if (!userOverride) return join4(homedir3(), ".codex", "lsp-client.json");
...
function isServerInstalled(command, _workingDirectory) {
	...
	if (cmd.includes("/") || cmd.includes("\\")) {
		if (existsSync5(cmd)) return true;
	}
...
function realpathForValidation(filePath) {
	if (existsSync8(filePath)) return realpathSync(filePath);
