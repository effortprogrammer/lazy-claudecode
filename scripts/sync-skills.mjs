...
import { sharedSkillsRootPath } from "@effortprogrammer/shared-skills";
...
const skillDisplayPrefix = "(LazyClaude) ";

const opencodeOnlyOrchestrationPattern = /\b(?:call_lazy-claudecode_agent|background_output|team_[a-z_]+|task)\s*\(/;

const claudeHarnessToolCompatibility = `## Claude Code Harness Tool Compatibility

This skill may include examples copied from the OpenCode harness. In Claude Code, do not call OpenCode-only tools such as \`call_lazy-claudecode_agent(...)\`, \`task(...)\`, \`background_output(...)\`, or \`team_*(...)\` literally. Translate those examples to Claude Code native tools:

| OpenCode example | Claude Code tool to use |
| --- | --- |
| \`call_lazy-claudecode_agent(subagent_type="explore", ...)\` | \`multi_agent_v1.spawn_agent({"message":"TASK: act as an explorer. ...","agent_type":"explorer","fork_context":false})\` |
| \`call_lazy-claudecode_agent(subagent_type="librarian", ...)\` | \`multi_agent_v1.spawn_agent({"message":"TASK: act as a librarian. ...","agent_type":"librarian","fork_context":false})\` |
| \`task(subagent_type="plan", ...)\` | \`multi_agent_v1.spawn_agent({"message":"TASK: act as a planning agent. ...","agent_type":"plan","fork_context":false})\` |
| \`task(subagent_type="oracle", ...)\` for final verification | \`multi_agent_v1.spawn_agent({"message":"TASK: act as a rigorous reviewer. ...","agent_type":"lazy-claudecode-gate-reviewer","fork_context":false})\` |
| \`task(category="...", ...)\` for implementation or QA | \`multi_agent_v1.spawn_agent({"message":"TASK: act as an implementation or QA worker. ...","fork_context":false})\` |
| \`background_output(task_id="...")\` | \`multi_agent_v1.wait_agent(...)\` for mailbox signals |
| \`team_*(...)\` | Use Claude Code native subagents via \`multi_agent_v1.spawn_agent\`, \`multi_agent_v1.send_input\`, \`multi_agent_v1.wait_agent\`, and \`multi_agent_v1.close_agent\` |

Role-specific behavior must be described in a self-contained \`message\`. Use \`fork_context: false\` to start the child with only the initial prompt (no parent history); use \`fork_context: true\` only when full parent history is truly required. Include any required conversation context, files, diffs, constraints, and requested skill names directly in the spawned agent's \`message\`. LAZY_CLAUDECODE installs these selectable agent roles into \`~/.claude/agents/\`: \`explorer\`, \`librarian\`, \`plan\`, \`momus\`, \`metis\`, \`lazy-claudecode-code-reviewer\`, \`lazy-claudecode-qa-executor\`, and \`lazy-claudecode-gate-reviewer\` - pass the matching name as \`agent_type\` so the child gets that role's model and instructions. If the spawn tool exposes no \`agent_type\` parameter, omit it and describe the role inside \`message\`. If a code block below conflicts with this section, this section wins.

For work likely to exceed one wait cycle, require the child to send \`WORKING: <task> - <current phase>\` before long passes and \`BLOCKED: <reason>\` only when progress stops. A \`multi_agent_v1.wait_agent\` timeout only means no new mailbox update arrived. Treat a running child as alive. Fallback only when the child is completed without the deliverable, ack-only after followup, explicitly \`BLOCKED:\`, or no longer running.

`;

const claudeCompatibilityEndMarkers = [
	"For work likely to exceed one wait cycle, require the child to send `WORKING: <task> - <current phase>` before long passes and `BLOCKED: <reason>` only when progress stops. A `multi_agent_v1.wait_agent` timeout only means no new mailbox update arrived. Treat a running child as alive. Fallback only when the child is completed without the deliverable, ack-only after followup, explicitly `BLOCKED:`, or no longer running.\n\n",
...
function findClaude CodeCompatibilitySectionEnd(content, searchStart) {
	...
	const knownEndMarker = claudeCompatibilityEndMarkers.find((marker) => content.indexOf(marker, searchStart) !== -1);
	if (knownEndMarker === undefined) return content.length;

	return content.indexOf(knownEndMarker, searchStart) + knownEndMarker.length;
}

function removeClaude CodeCompatibilityGuidance(content) {
	const heading = "## Claude Code Harness Tool Compatibility";
	...
	while (true) {
		...
		const end = findClaude CodeCompatibilitySectionEnd(withoutGuidance, start + heading.length);

		withoutGuidance = `${withoutGuidance.slice(0, start)}${withoutGuidance.slice(end)}`;
	}
}

function hasKnownGeneratedClaude CodeCompatibilityGuidance(content, compatibilityIndex) {
	return claudeCompatibilityEndMarkers.some((marker) => content.indexOf(marker, compatibilityIndex) !== -1);
}

export function insertClaude CodeCompatibilityGuidance(content) {
	...
	const compatibilityIndex = content.indexOf("## Claude Code Harness Tool Compatibility");
	if (
		compatibilityIndex !== -1 &&
		compatibilityIndex < firstExampleIndex &&
		!hasKnownGeneratedClaude CodeCompatibilityGuidance(content, compatibilityIndex)
	) {
		return content;
	}

	const contentWithoutGuidance = removeClaude CodeCompatibilityGuidance(content);

	const frontmatterMatch = contentWithoutGuidance.match(/^---\n[\s\S]*?\n---\n+/);
	if (!frontmatterMatch) {
		return `${claudeHarnessToolCompatibility}${contentWithoutGuidance}`;
	}

	return `${frontmatterMatch[0]}${claudeHarnessToolCompatibility}${contentWithoutGuidance.slice(frontmatterMatch[0].length)}`;
}
...
2. If worktree mode was used, sync \`.lazy-claudecode/\` state back to the main repo, merge or hand off exactly as requested, and remove the worktree only after successful merge or explicit handoff.
...
const startWorkClaude CodeCompletion = `When all top-level checkboxes in \`## TODOs\` and \`## Final Verification Wave\` are complete:
...
2. Complete the **Global Review and Debugging Gate** before any completion claim, PR handoff, or branch handoff:
   ...
   - Run a debugging-oriented runtime audit even when the review passes: name at least three plausible failure hypotheses for the changed surface, run the distinguishing checks against the actual artifact, and append the ruled-out or confirmed result to \`.lazy-claudecode/start-work/ledger.jsonl\`.
   ...
   - Evidence hygiene is mandatory: redact or mask secrets and sensitive user data before writing \`.lazy-claudecode/start-work/ledger.jsonl\`, a PR body, or a handoff. Never include raw tokens, credentials, auth headers, cookies, API keys, env dumps, private logs, or PII; use concise summaries, lengths, hashes, or short non-sensitive prefixes instead.
...
3. If worktree mode was used, sync \`.lazy-claudecode/\` state back to the main repo, merge or hand off exactly as requested, and remove the worktree only after successful merge or explicit handoff.
...
const reviewWorkClaude CodeGate = `
...
function applyClaude CodeSkillOverlays(skillName, content) {
	if (skillName === "start-work") {
		return content
			.replace(startWorkOriginalCompletion, startWorkClaude CodeCompletion)
			.replace(startWorkOriginalHardRule, startWorkClaude CodeHardRule);
	}
	if (skillName === "review-work" && !content.includes("When `review-work` is used as a final implementation")) {
		return content.replace(reviewWorkAnchor, `${reviewWorkAnchor}${reviewWorkClaude CodeGate}`);
	}
	return content;
}
...
async function writeClaude CodeSkillDisplayMetadata(skillName) {
	...
	const metadataPath = join(metadataDir, "claude.yaml");
...
async function adaptSkillForClaude Code(skillName) {
	const skillPath = join(skillsRoot, skillName, "SKILL.md");
	const content = await readFile(skillPath, "utf8");
	const adapted = applyClaude CodeSkillOverlays(skillName, insertClaude CodeCompatibilityGuidance(content));
	if (adapted !== content) {
		await writeFile(skillPath, adapted, "utf8");
	}
	await writeClaude CodeSkillDisplayMetadata(skillName);
}
...
async function syncSkills() {
	...
	for (const [name, source] of skillSources) {
		...
		await adaptSkillForClaude Code(name);
	}
	...
	for (const skillName of sharedSkillNames) {
		...
		await adaptSkillForClaude Code(skillName);
	}
}
