...
async function main() {
  switch (command) {
    case "install": {
      const { install } = await import("../dist/installer/install.js");
      await install(ROOT);
      break;
    }
    case "uninstall": {
      const { uninstall } = await import("../dist/installer/uninstall.js");
      await uninstall(ROOT);
      break;
    }
    case "doctor": {
      const { doctor } = await import("../dist/installer/doctor.js");
      await doctor(ROOT);
      break;
    }
    case "version":
